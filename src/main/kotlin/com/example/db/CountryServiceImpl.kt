...
class CountryServiceImpl : CountryService {
    ...
    override suspend fun addCountry(country: Country): Country?  = dbQuery {
        val insertStmt=Countries.insert {
            it[id] = country.cId
    ...
    override suspend fun getAllCountries(): List<Country> = dbQuery {
        Countries.selectAll().map { resultRowToCountry(it) }
    }

    override suspend fun deleteCountry(id: String): Boolean = dbQuery {
        TODO("Investigar como realizar el delete en Exposed")
    }

    override suspend fun getCountry(id: String): Country? = dbQuery {
